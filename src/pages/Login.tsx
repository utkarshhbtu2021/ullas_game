...
const Login: React.FC = () => {
  ...
  useEffect(() => {
    if (!hasSpoken) {
      let loginText = "";
      if (language === "hi") {
        loginText = "लॉगिन करें और अपने खाते में प्रवेश करें। ";
      } else {
        loginText = "Login to start learning.";
      }
      speak(loginText, true);
      setHasSpoken(true);
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (success) {
        ...
        const errorMsg =
          language === "hi"
            ? "गलत नाम या उपयोगकर्ता नाम"
            : "Invalid name or username";
        setError(errorMsg);
        speak(errorMsg);
      }
    } catch (error) {
